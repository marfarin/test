...
use common\components\UserAuthEvent;
...
class AuthController extends BaseController
{
    ...
    public function actionConfirmRegistrationEmail($token)
    {
        if (ConfigureRbac::getActualConfigureRbacInstance()->email_confirmation_required) {
            ...
            throw new NotFoundHttpException(Yii::t('app', 'Token not found. It may be expired'));
        }
    }
    ...
    public function actionConfirmEmail()
    {
        ...
        if ($model->load(Yii::$app->request->post()) && $model->validate()) {
            if ($this->triggerModuleEvent(UserAuthEvent::BEFORE_EMAIL_CONFIRMATION_REQUEST, ['model' => $model])) {
                if ($model->sendEmail(false)) {
                    if ($this->triggerModuleEvent(
                        UserAuthEvent::AFTER_EMAIL_CONFIRMATION_REQUEST,
                        ['model' => $model]
                    )
                    ) {
                        return $this->refresh();
                    ...
                    Yii::$app->session->setFlash(
                        'error',
                        Yii::t('app', "Unable to send message for email provided")
                    );
                }
            }
        }
